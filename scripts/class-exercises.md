...
Alignment was printed to /home/laurel/Downloads/genes_FAA/4471/4471-muscle.fa.uniqueseq.phy

For your convenience alignment with unique sequences printed to /home/laurel/Downloads/genes_FAA/4471/4471-muscle.fa.uniqueseq.phy
...
All model information printed to /home/laurel/Downloads/genes_FAA/4471/4471-muscle.fa.model.gz
...
Analysis results written to: 
  IQ-TREE report:                /home/laurel/Downloads/genes_FAA/4471/4471-muscle.fa.iqtree
  Maximum-likelihood tree:       /home/laurel/Downloads/genes_FAA/4471/4471-muscle.fa.treefile
  Likelihood distances:          /home/laurel/Downloads/genes_FAA/4471/4471-muscle.fa.mldist
  Screen log file:               /home/laurel/Downloads/genes_FAA/4471/4471-muscle.fa.log
...
###Bayesian Inference
Bayesian inference conducted using Mr. Bayes.
```
sudo apt-get update
sudo apt-get -y install libmrpt-bayes2.4
```
Beagle was not working so I installed beagle to /usr/bin and added it to the PATH by adding the location to the .bashrc file.
```
sudo apt-get update
...
```
Convert my data from a FASTA file to a NEXUS file. I had to install pip first before installing seqconverter. See an example of converting the MUSCLE MSA of gene 4471 from FASTA to NEXUS.
```
sudo apt install python3-pip
python3 -m pip install seqconverter
seqconverter --informat fasta --outformat nexus -i ~/Downloads/genes_FAA/4471/4471-MUSCLE/4471-muscle.fa > 4471-muscle.nexus
```
I create a Mr. Bayes text block in a separate text file in the same file as the MSA NEXUS file called mbblock.txt.
```
nano mbblock.txt
```
In the text file:
...
```
I then append this to the end of the NEXUS file, 4471-muscle.nexus for example here.
```
cat 4471-muscle.nexus mbblock.txt > 4471-muscle-mb.nexus
```
Now, run Mr. Bayes.
```
mb 4471-muscle-mb.nexus
```
The same process is repeated for the KALIGN file of the same gene and the other subset of genes chosen for this project.
...
Install ASTRAL by downloading the zip file and extracting the contents. For me, I download and extract all software to ~/Downloads/Programs. ASTRAL can be run from any directory with:
```
java -jar ~/Downloads/Programs/Astral.5.7.8/Astral/astral.5.7.8.jar
```
To execute:
```
java -jar ~/Downloads/Programs/Astral.5.7.8/astral.5.7.8.jar -i in.tree -o out.tre 2>out.log
```
Note that in.tree is the input gene trees in Newick format in one file, which can have missing taxa or unresolved branch lengths. Note that out.tre is the output file and the parameter 2>out.log will save the results in a file. Output of ASTRAL should be treated as an unrooted tree.
It is important to score a completed ASTRAL tree. Use the -q option to score a species tree and produce its quartet score, compute branch lengths, and compute ASTRAL branch support values. The score is converted into a normalized quartet score (a fraction of the induced quartet trees in the input set that are in the species tree). Ex: A score of 0.9 means 90% of the quarted trees induced by the gene trees are in the final species tree.
Run the commands below and receive a Final Quarted Score and a Final Normalized Quarted Score:
``` java -jar ~/Downloads/Programs/Astral.5.7.8/Astral/astral.5.7.8.jar -i in.tree -q out.tre -o scored_out.tre 2> scored_out.log
```
I do not yet have my own data compiled into one Newick file as my datasets are quite large and are still being run through the Maximum Likelihood method (iqtree), but when that is done, 1 file will be created with all of the MUSCLE data and 1 for the KALIGN data and they will be run through ASTRAL using the above code.
For MUSCLE:
```
java -jar ~/Downloads/Programs/Astral.5.7.8/Astral/astral.5.7.8.jar -i ~/botany-project/muscle-ml-8.gene.tre -o muscle-species.tre 2>muscle-species.tre
```
For KALIGN:
```
java -jar ~/Downloads/Programs/Astral.5.7.8/Astral/astral.5.7.8.jar -i ~/botany-project/kalign-ml-8.gene.tre -o kalign-species.tre 2>kalign-species.tre
```



###Coestimation with BEAST
Install BEAST2 and the other required packages.
```
sudo apt update
sudo apt install beast2-mcmc
```
Download Tracer_v1.7.2.tgz from Github and extract to install Tracer.
Downloads FigTree_v1.4.4.tgz from Github and extract to install FigTree.
Downloads sample data from the tutorial website by right-clicking on the link to the data file and doing "Save As." Ensure the file is saved as a .nex and save to the phylogeneticss-class-clone/exercises folder in your repository. The data is called primate-mtDNA.nex

We begin by creating an html analysis file in BEAUTi before we are able to use BEAST2. The analysis file contains the data, model specification, initial values and parameter constraints, settings of the MCMC algorithms, and output options. Open BEAUti:
```
cd ~/bin/beauti2
```
In your files, navigate to the location of primate-mtDNA.nex, and drag and drop it from your files onto the "Partition" tab of the BEAUti window. The data should appear. Make sure all partitons share the same evolutionary history. Press SHIFT and CLICK on all four partitions, and CLICK the LINK TREES and LINK CLOCK MODELS buttons. Both the CLOCK MODEl and TREE colums should change to say "noncoding." Now rename both so that the output file is easier to read. In the dropdown menu under CLOCK MODEL, delete "noncoding" and type "clock" and hit enter so it changes all rows to clock. Do the same under the TREE colums but type "tree."

Next, we will set the substitution model. Select the SITE MODEL tab. We think they all evolved under different substitution models but according to the same general model, so we treat them all separately. Check the ESTIMATE checkbox for SUBSTIUTION RATE. Set the GAMMA CATEGORY COUNT to 4. Check the ESTIMATE box for SHAPE parameter. Select HKY in the SUBST MODEL drop-down menu. Ensure the ESTIMATE box for KAPPA is checked. Select EMPIRICAL from the FREQUENCIES drop-down menu.
